...
public class Medicine(Guid medicationTypeId, string title, string description, string specimen) : AuditableEntity<Guid>
{
    ...
    /// </summary>
    public string Format { get; private set; } = specimen;

    public static Medicine Default => new(Guid.Empty, string.Empty, string.Empty, string.Empty);

    public virtual MedicationType MedicationType { get; set; } = MedicationType.Default;
