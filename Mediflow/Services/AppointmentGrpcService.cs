...
public class AppointmentGrpcService : AppointmentService.AppointmentServiceBase
{
        ...
        ServerCallContext context)
    {
        try
        {
            ...
            {
                throw new Exception("Invalid appointment ID format");
            }

            var appointment = await _unitOfWork.Appointments.GetByIdAsync(request.Id);
            if (appointment == null)
            {
                throw new Exception($"Appointment not found with ID: {request.Id}");
            }
        ...
        ServerCallContext context)
    {
        try
        {
            ...
            if (appointment == null)
            {
                throw new Exception($"Appointment {request.Id} not found");
            }
                ...
                appointment.MedicalRecordId == null)
            {
                ...
                {
                    throw new Exception("Notes (Diagnosis) and Symptoms (Treatment) are required to complete appointment");
                }
